...
//		}
	...
	public Aluno[] ordenarAlunos() {
		...
		for (int i = 0; i < resposta.length - 1; i++) {
			for (int j = i; j < resposta.length; j++) {
				String matricula1 = formarMatricula(resposta, i);
				String matricula2 = formarMatricula(resposta, j);
	...
	private String formarMatricula(Aluno[] resposta, int i) {
		String matricula = resposta[i].getPrefixoMatricula() + "/" + resposta[i].getSufixoMatricula();
		return matricula;
	}
